...
export default function HeroSection({ profile }) {
  ...
  const fullName = profile?.full_name || "Harsha C";
  const headline = profile?.headline || "Creative Developer & Designer";

  return (
    <section 
      id="hero" 
      ref={containerRef}
      className="relative min-h-[100dvh] w-full flex flex-col items-center justify-between overflow-hidden bg-transparent py-20 text-white"
    >
      ...
      <div className="relative z-10 w-full flex flex-col items-center justify-between flex-1 select-none pt-24 pb-8">
        <motion.div 
        ...
        >
          <div className={`absolute inset-0 bg-white/5 blur-xl transition-all duration-700 rounded-full group-hover:bg-opacity-20`} style={{ backgroundColor: isHovered ? activeTheme.sky.start : '' }} />
          <span className="relative font-serif text-xs font-bold tracking-[0.25em] uppercase text-white drop-shadow-md px-6 py-2 border border-white/10 rounded-full backdrop-blur-sm">
            {headline}
          </span>
        </motion.div>
        ...
        >
          ...
          >
            ...
            <h1 
              className="font-serif text-[4rem] sm:text-[8rem] md:text-[10rem] font-light leading-none text-center select-none tracking-tight flex text-transparent"
              style={{ WebkitTextStroke: "1px rgba(255, 255, 255, 0.5)" }}
            >
              {Array.from(fullName).map((letter, i) => (
                <motion.span 
                  key={`stroke-${i}`} 
                  className={letter === " " ? "w-4 sm:w-10" : "inline-block"}
                  initial={{ opacity: 0, y: 60, scale: 0.9, filter: "blur(12px)" }}
                  animate={{ opacity: 1, y: 0, scale: 1, filter: "blur(0px)" }}
                  transition={{ duration: 1.6, delay: 0.8 + i * 0.1, ease: [0.16, 1, 0.3, 1] }}
                >
                  {letter}
                </motion.span>
              ))}
            ...
            <h1 
              className="absolute inset-0 font-serif text-[4rem] sm:text-[8rem] md:text-[10rem] font-light leading-none text-center select-none tracking-tight flex text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.4)] pointer-events-none"
              style={{
