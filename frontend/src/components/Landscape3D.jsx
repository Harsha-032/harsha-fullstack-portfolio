...
const Terrain = () => {
  ...
  const { geometry } = useMemo(() => {
    // Massively increased plane size to prevent edges from showing
    const geo = new THREE.PlaneGeometry(1600, 4000, 128, 256); 
    geo.rotateX(-Math.PI / 2);
    // Center it better across the camera's Z travel path (camera goes from +80 to -570)
    geo.translate(0, 0, -1000);
...
const AtmosphericEffects = ({ scrollProgress }) => {
  ...
  // Create colors from theme but we need to keep instance refs outside useFrame or recreate inside
  // It's safe to just recreate or update them
...
const River = () => {
  ...
  const geometry = useMemo(() => {
    const geo = new THREE.PlaneGeometry(80, 4000, 16, 256); 
    geo.rotateX(-Math.PI / 2);
    geo.translate(0, 0, -1000);
