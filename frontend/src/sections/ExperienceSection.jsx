  ...
  const fallbackExperiences = [
    {
      ...
      description: "Leading the development of highly scalable microservices architecture using Node.js and Golang. Implemented dynamic automated workflows and integrated AI modules.",
      employment_type: "Full-Time"
    }
  ];

  const displayExperiences = experiences?.length ? experiences : fallbackExperiences;
    ...
    >
      ...
      >
        ...
        <div className="relative w-full">
          ...
          <motion.div 
            className={`absolute left-6 md:left-1/2 top-0 w-0.5 md:-translate-x-1/2 origin-top shadow-[0_0_10px_rgba(255,255,255,0.5)]`}
            style={{ height: lineHeight, backgroundColor: activeTheme.sky.start }}
          />
              ...
              return (
                <div key={i} className={`relative flex items-center justify-start md:justify-between w-full group ${!isEven ? 'md:flex-row-reverse' : ''}`}>
                  ...
                  <motion.div 
                    ...
                    className="absolute left-6 md:left-1/2 -translate-x-1/2 flex items-center justify-center w-10 h-10 md:w-14 h-14 rounded-full bg-black border-2 border-white/20 group-hover:border-white/60 transition-colors duration-300 z-10"
                  >
