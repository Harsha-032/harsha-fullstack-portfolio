...
export default function ContactSection({ profile, socialLinks }) {
  ...
  const email = profile?.email || "harshac032@gmail.com";
  const location = profile?.location || "India (Remote)";
                ...
                {socialLinks?.map((link, i) => (
                  <React.Fragment key={link.id}>
                    <a 
                      ...
                      target="_blank"  
                      rel="noreferrer"
                      className={`text-white ${activeTheme.ui.textHover} flex items-center gap-1 transition-colors group uppercase`}
                      data-cursor="pointer"
                    >
                      {link.platform}
                      <ArrowUpRight className="w-3 h-3 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                    </a>
                    {i < socialLinks.length - 1 && <span className="text-white/20">/</span>}
                  </React.Fragment>
                ))}
                {!socialLinks?.length && (
                  <span className="text-white/40">No social links</span>
                )}
              </div>
            </div>
          </div>
    ...
    <AnimatePresence>
      ...
      {status === "error" && (
        ...
        >
          ...
          <div>
            <h5 className="font-sans text-sm font-bold text-red-300 leading-none">Error Submitting</h5>
